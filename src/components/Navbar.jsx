...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-base-200 text-white shadow-md">
          ...
          {menuItems.map((item) =>
            item.sub ? (
              <div className="dropdown dropdown-hover" key={item.name}>
                ...
                <ul
                  tabIndex={0}
                  className="dropdown-content menu p-2 shadow bg-base-200 text-white rounded-box w-52"
                >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden fixed top-0 right-0 w-72 h-full bg-base-200 text-white shadow-lg z-40 overflow-y-auto px-4 pt-20 pb-6 space-y-4"
          >
            {menuItems.map((item) =>
              item.sub ? (
                <div key={item.name}>
                  <div
                    className="flex items-center justify-between px-2 py-2 font-semibold cursor-pointer hover:bg-base-300 rounded-md"
                    onClick={() =>
                      setOpenDropdown(
                  ...
                  <AnimatePresence>
                    {openDropdown === item.name && (
                      <motion.div
                        ...
                        className="ml-4 pl-2 border-l border-gray-500 space-y-1 mt-1"
                      >
